...
async function initialize() {
    const promMordi = await fetch("https://capsules-asb6.herokuapp.com/api/teacher/mordi");
    const promToam = await fetch("https://capsules-asb6.herokuapp.com/api/teacher/toam");
    const mordiData = await promMordi.json();
    const toamData = await promToam.json();
    const data = mordiData.concat(toamData);
    console.log(data);
    for(let student of data) {
        const prom = await fetch(`https://capsules-asb6.herokuapp.com/api/user/${student.id}`);
